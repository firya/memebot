...
// If cfg.TelegramAPIURL is set, all requests go through that proxy instead of api.telegram.org.
func fetchImageBytes(cfg Config, fileID string) ([]byte, string, error) {
	base := "https://api.telegram.org"
	if cfg.TelegramAPIURL != "" {
		base = strings.TrimRight(cfg.TelegramAPIURL, "/")
	}

	tgFileURL := fmt.Sprintf("%s/bot%s/getFile?file_id=%s", base, cfg.TelegramToken, fileID)
	fileReq, err := http.NewRequest("GET", tgFileURL, nil)
	if err != nil {
		return nil, "", fmt.Errorf("getFile request: %w", err)
	}
	if cfg.TelegramWorkerSecret != "" {
		fileReq.Header.Set("X-Worker-Secret", cfg.TelegramWorkerSecret)
	}
	resp, err := tgAPIClient.Do(fileReq)
	if err != nil {
		return nil, "", fmt.Errorf("getFile request: %w", err)
	}
	...
	imageURL := fmt.Sprintf("%s/file/bot%s/%s", base, cfg.TelegramToken, fileInfo.Result.FilePath)
	imgReq, err := http.NewRequest("GET", imageURL, nil)
	if err != nil {
		return nil, "", fmt.Errorf("download request: %w", err)
	}
	if cfg.TelegramWorkerSecret != "" {
		imgReq.Header.Set("X-Worker-Secret", cfg.TelegramWorkerSecret)
	}
	imgResp, err := tgDownloadClient.Do(imgReq)
	if err != nil {
